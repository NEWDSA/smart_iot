  ...
  <PageWrapper dense contentFullHeight fixedHeight contentClass="flex">
    ...
    <BasicTable class="w-3/4 xl:w-4/5" :clickToRowSelect="clickToRowSelect" @register="registerTable"
      ...
      <template #toolbar>
        ...
        <a-button v-if="hasPermission(['BatchOut_RegionDevice'])" type="primary" @click="handleout">批量删除</a-button>
...
import { DeviceDeptPageApi,DeviceDeptDeleteApi } from '@/api/facility/facility';
...
export default defineComponent({
  ...
  setup() {
    ...
    var pagination = reactive({ DeptId: 1, PageNum: 1, PageSize: 10 })
    const {
      createConfirm,
      createMessage
    } = useMessage();
    ...
    const [registerTable, { reload, getSelectRowKeys, setSelectedRowKeys, deleteTableDataRecord }] = useTable({
      ...
      columns,
      api: async (p) => {
        const { List, Total } = await DeviceDeptPageApi(p);
        return {
          List,
          Total
        }
    ...
    async function handleout() {
      ...
      if (toRaw(checkedKeys.value).length > 0) {
        ...
        try {
          await DeviceDeptDeleteApi(param)
        } finally {
          reload()
        }
