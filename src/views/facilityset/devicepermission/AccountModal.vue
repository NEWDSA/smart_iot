...
import { DeviceDeptSaveApi,DeviceFreeListApi } from '@/api/facility/facility';
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
      ...
      await deviceTree().then((res) => {
        ...
        for (let i = 0; i < res.length; i++) {
          TreeTableData.value.push(tree([res[i]])[0]);
          // if (res[i].SonData) {
    ...
    async function handleSubmit() {
      try {
        const params = {
          DeviceIds: toRaw(checkedKeys.value),
          DeptId: DeptId.value
        }
        // 判断所选内容是否不为空
        ...
        !unref(isUpdate) ? await DeviceDeptSaveApi(params) : ''
