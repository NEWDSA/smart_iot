<template>
  <PageWrapper contentFullHeight title="区域列表">
    <BasicTable :canResize="true" @register="registerTable">
      <template #tableTitle>
        <a-button v-if="hasPermission(['createNewArea_RegionList'])" preIcon="mdi:plus" @click="handleCreate"
          type="primary">创建新区域</a-button>
      </template>
...
export default defineComponent({
  ...
  setup() {
    ...
    async function handleDelete(record: Recordable) {
      ...
      await delRegion({ RegionId: record.RegionId }).then((res)=>{
