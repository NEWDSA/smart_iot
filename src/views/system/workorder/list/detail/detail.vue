              ...
              <div v-if="item.Files?.length > 0 && item.Files[0] != ''">
                ...
                <Upload v-model:file-list="item.FilesUrl" name="file"
                  ...
                  <template #itemRender="{ file, actions }">
                    <Space>
                      <span :style="file.status === 'error' ? 'color: red' : ''" class="font-semibold">{{ file.name
                      }}</span>
    ...
    <Modal v-model:visible="sceneStopModal" title="系统提示" :footer="null">
      <div class="p-5">
        ...
        <div class="flex mt-3" style="justify-content: end;">
          ...
          <div class="text-blue-600 rounded px-4 py-1" style="border:1px solid rgba(37, 99, 235,1)" @click="CheckCancel">
            取消</div>
        </div>
      </div>
